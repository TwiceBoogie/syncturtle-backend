...
public interface UserSettingsControllerSwagger {
    ...
    @Operation(summary = "Update user's username", responses = {
            ...
            @ApiResponse(responseCode = "409",
                    description = USERNAME_ALREADY_TAKEN,
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = ApiErrorResponse.class),
                            examples = @ExampleObject(value = ApiResponseExamples.API_ERROR_USERNAME_TAKEN_EXAMPLE)))
    })
    ResponseEntity<String> updateUsername(@Validated(SettingsRequestGroup.UsernameUpdateGroup.class) @RequestBody SettingsRequest request,
    ...
    ResponseEntity<AuthenticationResponse> updateEmail(@Validated(SettingsRequestGroup.EmailUpdateGroup.class) @RequestBody SettingsRequest request,
                                                       BindingResult bindingResult);
    ...
    @Operation(summary = "Update user's phone", responses = {
            @ApiResponse(responseCode = "200",
                    description = "Successful response",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = UserPhoneResponse.class))),
            @ApiResponse(responseCode = "400",
                    description = "Validation error",
                    content = {
                            @Content(mediaType = "application/json",
    ...
    ResponseEntity<UserPhoneResponse> updatePhone(@Validated(SettingsRequestGroup.PhoneUpdateGroup.class) @RequestBody SettingsRequest request,
                                                  BindingResult bindingResult);
    ...
    ResponseEntity<String> updateGender(@Validated(SettingsRequestGroup.GenderUpdateGroup.class) @RequestBody SettingsRequest request,
                                        BindingResult bindingResult);
    ...
    @Operation(summary = "Update user's profile pic", responses = {
            @ApiResponse(responseCode = "200",
                    description = "Successful response",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = ProfilePicResponse.class))),
    ...
    @Operation(summary = "Upload profile pics", responses = {
            @ApiResponse(responseCode = "200",
                    description = "Successful response",
                    content = @Content(mediaType = "application/json",
                            array = @ArraySchema(schema = @Schema(implementation = ProfilePicResponse.class)))),
            @ApiResponse(responseCode = "400",
                    description = "No files provided for upload or more than 10 pics uploaded",
                    content = {
                            @Content(mediaType = "application/json",
                                    schema = @Schema(implementation = ApiErrorResponse.class),
                                    examples = @ExampleObject(value = ApiResponseExamples.API_ERROR_FILES))
                    })
    })
