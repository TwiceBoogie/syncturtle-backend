...
/**
 * This interface defines methods for authentication and user management within the application.
 * Implementations of this interface handle user authentication, password management, and device verification.
 *
 * <p>Methods in this interface provide functionality to:</p>
 * <ul>
 ...
 *     <li>Handle forgotten usernames and passwords by sending OTPs for verification.</li>
 *     <li>Verify OTPs and reset user passwords.</li>
 *     <li>Perform device verification to enhance security.</li>
 * </ul>
 *
 * <p>Exceptions may be thrown to indicate various error conditions, such as user not found, invalid credentials,
 * or account lockout. These exceptions provide details about the error, which can be used to inform the client
 * application about the failure.</p>
 *
 * <p>Implementations of this interface encapsulate the business logic for user authentication and management,
 * providing a standardized API for interacting with user-related functionality throughout the application.</p>
 */
public interface AuthenticationService {
     ...
     * @throws ApiRequestException If the user corresponding to the authenticated user ID is not found.
     */
    ...
    /**
     * Process the login request, authenticates user, and generates a JWT Token upon successfully login.
     * if auth fails due to incorrect creds or any other reason, appropriate exceptions are thrown.
     *
     * @param request The authentication request containing the username and password.
     * @param bindingResult The result of the binding process between the authentication request and the controller method.
     *
     * @return A map containing the authenticated user and a JWT Token.
     *
     * @throws ApiRequestException If the user is not found or the account is not verified.
     * @throws NoRollbackApiRequestException If the user's deviceKey is not valid or the account is locked.
     */
