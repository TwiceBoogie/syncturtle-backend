...
public class TokenServiceImpl implements TokenService {
    ...
    public IssuedTokens issueTokens(User user, TokenProvenance provenance) {
        ...
        RefreshTokenValue rc = new RefreshTokenValue(
                refreshTokenString(pair.handle(), pair.secret()), rt.getExpiresAt());

        return new IssuedTokens(at, rc);
    }
    ...
    public IssuedTokens rotateRefreshToken(String refreshToken, TokenProvenance provenance) {
        ...
        RefreshTokenValue rc = new RefreshTokenValue(
                refreshTokenString(pair.handle(), pair.secret()), rtNext.getExpiresAt());

        return new IssuedTokens(at, rc);
    }

    private String refreshTokenString(String handle, String secret) {
