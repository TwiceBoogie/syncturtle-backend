...
path "secret/data/instance-service" {
...
path "database/creds/instance-service" {
  capabilities = ["read"]
}
...
path "auth/approle/role/instance-service/role-id" {
  capabilities = ["read"]
}

# Allow creating and reading Secret ID
path "auth/approle/role/instance-service/secret-id" {
  capabilities = ["create", "read"]
}
